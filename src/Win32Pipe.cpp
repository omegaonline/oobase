...
OOBase::Win32::LocalSocket::LocalSocket(HANDLE hPipe) :
		m_hPipe(hPipe),
		m_hReadEvent(NULL),
		m_hWriteEvent(NULL)
{
...
OOBase::Win32::LocalSocket::~LocalSocket()
{
	close();
}

int OOBase::Win32::LocalSocket::send(const void* buf, size_t len, const OOBase::timeval_t* timeout)
{
	if (!m_hWriteEvent)
	{
	...
	{
		DWORD dwWritten = 0;
		if (WriteFile(m_hPipe,cbuf,static_cast<DWORD>(len),&dwWritten,&ov))
		{
		...
		if (!GetOverlappedResult(m_hPipe,&ov,&dwWritten,TRUE))
		{
			DWORD ret = GetLastError();
			CancelIo(m_hPipe);
			return ret;
...
size_t OOBase::Win32::LocalSocket::recv(void* buf, size_t len, int* perr, const OOBase::timeval_t* timeout)
{
	assert(perr);
	*perr = 0;
	...
	{
		DWORD dwRead = 0;
		if (ReadFile(m_hPipe,cbuf,static_cast<DWORD>(total),&dwRead,&ov))
		{
		...
		if (!GetOverlappedResult(m_hPipe,&ov,&dwRead,TRUE))
		{
			*perr = GetLastError();
			CancelIo(m_hPipe);
			return (len - total);
...
void OOBase::Win32::LocalSocket::close()
{
	if (m_hPipe.is_valid())
...
/*OOBase::SOCKET OOBase::Win32::LocalSocket::async_steal(int* perr)
{
	...
	HANDLE hClone;
	if (!DuplicateHandle(GetCurrentProcess(),m_hPipe,GetCurrentProcess(),&hClone,0,FALSE,DUPLICATE_SAME_ACCESS))
	{
		*perr = GetLastError();
